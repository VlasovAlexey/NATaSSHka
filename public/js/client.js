...
// Обработчик для управления воспроизведением медиа в чате (аудио и видео через стандартный медиа плеер)
document.addEventListener('play', function(e) {
    const target = e.target;
    
    // Проверяем, что это медиа элемент и находится в контейнере сообщений
    if ((target.tagName === 'VIDEO' || target.tagName === 'AUDIO') && 
        target.closest('#messagesContainer')) {
        
        // Включаем звук на текущем медиа элементе
        target.muted = false;
        
        // Находим все медиа элементы в контейнере сообщений, кроме текущего
        const allMedia = document.querySelectorAll('#messagesContainer video, #messagesContainer audio');
        allMedia.forEach(media => {
            if (media !== target && !media.paused) {
                // Ставим на паузу все другие воспроизводящиеся медиа
                media.pause();
            }
        });
    }
