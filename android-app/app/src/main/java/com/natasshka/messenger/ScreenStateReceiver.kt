...
class ScreenStateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            Intent.ACTION_SCREEN_ON -> {
                ...
                isScreenOn = true
                // Перезапускаем сервис при включении экрана
            ...
            Intent.ACTION_BOOT_COMPLETED -> {
                ...
                // Перезапускаем сервис после перезагрузки
                ChatForegroundService.startService(context)
            }
        }
    }
    ...
    private fun updateBackgroundState(context: Context) {
        // Отправляем системное сообщение о состоянии
        val appState = when {
        ...
        // Можно сохранить состояние в SharedPreferences для использования в MainActivity
